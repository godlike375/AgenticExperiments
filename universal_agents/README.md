...
- **Динамическое управление инструментами** — LLM может включать/отключать
  инструменты через `load_tools` и `unload_tool`, самостоятельно управляя
  ...
  `unload_tool` и `tool_description`, остальные инструменты агент
...
1. `main.py` загружает внешние инструменты через
   ...
   автоматически подгружаются `unload_tool` и `tool_description`;
...
class Config:
    ...
    BOOST_TEMP = 2                        # Температура при восстановлении после циклов/пустых ответов

    # Параметры генерации (используются по умолчанию во всех вызовах API)
    TEMP = 0.5               # Температура сэмплирования
...
- `unload_tool` — отключить активный инструмент по имени. Появляется автоматически после первого вызова `load_tools`.
