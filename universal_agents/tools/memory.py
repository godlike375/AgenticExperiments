"""Инструменты долговременной памяти: поиск/чтение архива вытесненных оригиналов (наполняется при компакции), чтобы модель могла ответить про удалённый из контекста фрагмент."""
