"""Чистые функции работы с tool call: извлечение имени/аргументов из разных
форматов, распознавание сломанных вызовов, парсинг и нормализация JSON-аргументов.

Ни одна функция здесь не зависит от LLMAgent — модуль не создаёт циклических импортов.
"""
...
def detect_broken_call(content: str, tool_names: set[str]) -> bool:
    ...
    Иерархия признаков (базовый гейт — наличие открывающегося XML-тега):
      ...
      2) наличие любого XML-тега + признак вызова известного инструмента
         (``имя_тула + скобка``, напр. ``read({...})`` или ``load_tool(``).
