...
SECTION_GUIDE = """
# Правила заполнения
- R1 Одно владение фактом: факт формулируется в одной секции; в других местах используются указатели («см. §4, Q2»).
- R2 Пустые секции не пропускаются - пишется «нет».
- R3 Заполняется всё исчерпывающе подробно.

* 1.1 Контекст: имена сущностей, роли, связи (без логики кода и номеров строк — это §4)
* 1.2 Специфические термины/сокращения/договорённости в рамках проекта/задачи (каждый определяется один раз — здесь).
* 1.3 Главная задача LLM, критерии выполнения (чек-лист), ожидаемый результат/артефакт (единственное место для критериев; §2.2 ссылается на них по номеру).
* 1.4 Ограничения и требования главной задачи LLM (строгие и нестрогие отдельно), дополнительные инструкции пользователя.
* 1.5 Постоянные предпочтения пользователя.

* 2.1 Выполненные шаги, результаты, трудности: «сделал X → результат Y» (выводы и детали в §4/§7).
* 2.2 Следующий и дальнейшие шаги, условия выполнения каждого шага, ожидаемый результат каждого шага. Привязка по ID к критерию (§1.3) или вопросу (§4.1).
* 2.3 Ключевые решения, неудачные попытки, отклонённые альтернативы (решение → причина → указатель; коррекция, меняющая вывод, одновременно обновляет утверждение §4.1)

* 3.1 Ждущие задачи и зависимости, кто их блокирует (пользователь/система) и почему. Пусто = «нет»

* 4.1 Реестр утверждений со статусами
- [C#] подтверждено: ... (доказательство → §6)
- [Q#] открытый вопрос: что именно проверить и где (место, критерии соответствия)
- [H#] гипотеза: формулировка + как её опровергнуть

* 5.1 Мета-данные о задаче, проекте, пользователе (то что не сказано/написано напрямую). 

* 6.1 Задействованные ресурсы/артефакты, незавершённые черновики (созданные/изменённые файлы + внешние ресурсы для повторного доступа). Для каждого: зачем, где, мета-данные, назначение.

* 7.1 Подсказки для продолжения работы другой LLM.
* 7.2 Прочая (только важная) информация (либо её отсутствие).

"""

NEW_TO_PREVIOUS = """
*   7.3 Новое относительно предыдущего саммари (обязательно заполняется).
"""
...
def summarize_history_plain(
    ...
    msg_obj, err, usage = agent.service_llm_call(msgs, temp=temp, timeout=Config.TIMEOUT, prefill='* 1.1 ')
    if err or not msg_obj or not msg_obj.content:
