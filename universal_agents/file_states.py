"""Реестр состояний прочитанных файлов.

Позволяет экономить контекст: запоминает хэш содержимого файла и привязку к
результату инструмента 'read' в истории. Если при повторном read хэш не изменился
и результат прошлого чтения всё ещё цел в истории (не стёрт и не сжат), повторное
чтение пропускается.
"""
...
class FileStateTracker:
    """Хранит маппинг «путь → (хэш содержимого, tool_call_id read-результата)».

    Хранится в оперативной памяти агента. Сохраняется/восстанавливается вместе с
    историей через to_dict()/from_dict(). Валидация «целостности» выполняется по
    живой истории: если read-результат стёрт из истории или его контент сжат,
    запись помечается недействительной (следующий read реально перечитает файл).
    """
    ...
    def record(self, path: str, disk_hash: str, content_hash: str) -> None:
        """Запоминает состояния для пути (привязка к tool_call_id позже).

        disk_hash — хэш сырого содержимого файла (детекция изменения на диске).
        content_hash — хэш того, что реально попало в историю (валидация того,
                       что контент ещё цел в истории, а не стёрт/сжат).
        """
